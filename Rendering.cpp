...
RenderedModel renderedModels[1000];
...
void Rendering::start(Entities &entities) {
    initWindow();
    initVulkan();
    initImgui();
    initEngine();
...
void Rendering::initWindow() {
    ...
    window = glfwCreateWindow(WIDTH, HEIGHT, ENGINE_TITLE.c_str(), nullptr, nullptr);
    glfwSetWindowUserPointer(window, this);
    glfwSetFramebufferSizeCallback(window, framebufferResizeCallback);
}

void Rendering::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
...
void Rendering::mainLoop() {
    ...
    EngineUI::initUI(this);

    while (!glfwWindowShouldClose(window) && applicationOpen) {
        glfwPollEvents();

        //editor camera controls
        if (EngineUI::getIsNavigatingViewport()) {
        ...
        ImGui::NewFrame();
        EngineUI::loopUI(applicationOpen, this, cameraSpeed, fov, firstMouse);
        ImGui::EndFrame();
...
void Rendering::cleanupSwapChain() {
...
void Rendering::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
    createInfo = {};
...
void Rendering::createGraphicsPipeline() {
...
void Rendering::createCommandPool() {
...
void Rendering::createColorResources() {
...
void Rendering::createDepthResources() {
...
bool Rendering::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

void Rendering::createTextureImage() {
...
void Rendering::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
void Rendering::createTextureImageView() {
    textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

void Rendering::createTextureSampler() {
...
VkImageView Rendering::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void Rendering::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void Rendering::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Rendering::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Rendering::createUniformBuffers() {
...
void Rendering::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void Rendering::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    ...
    for (size_t i = 0; i < size; i++)
    {
        if (renderedModels[i].vertexBuffer != NULL) {
            VkBuffer vertexBuffers[] = { renderedModels[i].vertexBuffer };
            VkDeviceSize offsets[] = { 0 };
            vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers, offsets);
            vkCmdBindIndexBuffer(commandBuffer, renderedModels[i].indexBuffer, 0, VK_INDEX_TYPE_UINT32);
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &renderedModels[i].descriptorSets[currentFrame], 0, nullptr);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(renderedModels[i].indices), 1, 0, 0, 0);
        }
    }
...
void Rendering::updateUniformBuffer(uint32_t currentImage) {
    ...
    //This is our camera
    // Calculate the view and projection matrices based on the camera position and orientation
...
VkExtent2D Rendering::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
SwapChainSupportDetails Rendering::querySwapChainSupport(VkPhysicalDevice device) {
...
bool Rendering::isDeviceSuitable(VkPhysicalDevice device) {
...
QueueFamilyIndices Rendering::findQueueFamilies(VkPhysicalDevice device) {
...
std::vector<char> Rendering::readFile(const std::string& filename) {
...
bool Rendering::LoadTextureFromFile(const char* filename, ImGuiTextureData* tex_data)
{
...
void Rendering::RemoveTexture(ImGuiTextureData* tex_data)
{
...
void Rendering::updateCamera(GLFWwindow* window, float deltaTime)
{
...
Any function postfixed with NEW is a new function I have created to replace the old function.
...
void Rendering::createVulkanMemoryAllocator() {
...
VkImage Rendering::createTextureImageNEW(std::string texturePath) {
    int texWidth, texHeight, texChannels;
...
VkSampler Rendering::createTextureSamplerNEW() {
...
void Rendering::loadModelDynamically(std::string modelPath, std::string texturePath) {
   ...
   // VkDeviceMemory stagingBufferMemory;
    ...
    void* data;
    int emptySlot = 0;
    int size = sizeof(renderedModels) / sizeof(renderedModels[0]); //Find empty slot for this model to occupy
    for (size_t i = 0; i < size; i++) 
    {
        if (renderedModels[i].vertexBuffer == NULL) {
            emptySlot = i;
    ...
    //LOAD THE MODEL: Load the model's vertex and index data
    tinyobj::attrib_t attrib;
    std::vector<tinyobj::shape_t> shapes;
    std::vector<tinyobj::material_t> materials;
    std::string warn, err;
    ...
    indexBufferInfo.size = sizeof(modelIndices[0]) * modelIndices.size();
    ...
    newModel.indexBuffer = indexBuffer;
    newModel.indices = modelIndices.size();
    newModel.indexBufferMemory = indexBufferAllocation->GetMemory();
    newModel.vertexBufferMemory = vertexBufferAllocation->GetMemory();
    newModel.descriptorSets = descriptorSets;
    renderedModels[emptySlot] = newModel;
